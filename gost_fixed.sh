...
function check_new_ver() {
  # deprecated
  ct_new_ver=$(if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate -qO- -t2 -T3 https://api.github.com/repos/ginuerzh/gost/releases/latest | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g;s/v//g')
...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://gotunnel.oss-cn-shenzhen.aliyuncs.com/gost-linux-"$bit"-"$ct_new_ver".gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://gotunnel.oss-cn-shenzhen.aliyuncs.com/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://gotunnel.oss-cn-shenzhen.aliyuncs.com/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
    if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://github.com/ginuerzh/gost/releases/download/v"$ct_new_ver"/gost-linux-"$bit"-"$ct_new_ver".gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi --no-check-certificate https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
function cert() {
  ...
  if [ "$numcert" == "1" ]; then
    ...
    if [ $use_proxy -eq 1 ]; then curl --proxy socks5://$proxy_address:$proxy_port; else curl; fi https://get.acme.sh | sh
...
# Ask if the user wants to use a SOCKS5 proxy
use_proxy=0
proxy_address=""
proxy_port=""
...
update_sh() {
  ol_version=$(if [ $use_proxy -eq 1 ]; then curl --proxy socks5://$proxy_address:$proxy_port; else curl; fi -L -s --connect-timeout 5 https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [ -n "$ol_version" ]; then
    if [[ "$shell_version" != "$ol_version" ]]; then
      ...
      [yY][eE][sS] | [yY])
        if [ $use_proxy -eq 1 ]; then wget --no-check-certificate -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port; else wget; fi -N --no-check-certificate https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/gost.sh
        echo -e "更新完成"
        exit 0
        ;;
      *) ;;
