...
if [ $use_proxy -eq 1 ]; then
            execute_with_proxy wget -e use_proxy=yes -e http_proxy=socks5://$proxy_address:$proxy_port "$@"
        else
            execute_with_proxy wget "$@"
        fi
    elif [ "$command" == "execute_with_proxy curl" ]; then
        
if [ -z "$use_proxy" ]; then
    use_proxy=0
fi
if [ $use_proxy -eq 1 ]; then
            execute_with_proxy curl --proxy socks5://$proxy_address:$proxy_port "$@"
        else
            execute_with_proxy curl "$@"
        fi
    fi
}
#! /bin/bash
...
function check_new_ver() {
  # deprecated
  ct_new_ver=$(execute_with_proxy wget -qO- -t2 -T3 https://ghproxy.com/https://api.github.com/repos/ginuerzh/gost/releases/latest | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g;s/v//g')
...
" addyn
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    execute_with_proxy wget https://gotunnel.oss-cn-shenzhen.aliyuncs.com/gost-linux-"$bit"-"$ct_new_ver".gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    execute_with_proxy wget https://gotunnel.oss-cn-shenzhen.aliyuncs.com/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && execute_with_proxy wget https://gotunnel.oss-cn-shenzhen.aliyuncs.com/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
    execute_with_proxy wget https://ghproxy.com/https://github.com/ginuerzh/gost/releases/download/v"$ct_new_ver"/gost-linux-"$bit"-"$ct_new_ver".gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    execute_with_proxy wget https://ghproxy.com/https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && execute_with_proxy wget https://ghproxy.com/ghproxy.com/https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
function cert() {
  ...
  if [ "$numcert" == "1" ]; then
    ...
    execute_with_proxy curl https://get.acme.sh | sh
...
update_sh() {
  ol_version=$(execute_with_proxy curl -L -s --connect-timeout 5 https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
